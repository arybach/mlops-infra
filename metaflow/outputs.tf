...
data "aws_security_group" "metaflow_service_sg" {
  name = "metaflow-metadata-service-security-group-${var.env}-${lookup(module.common_vars.aws_regions, data.aws_region.current.name)}"
  depends_on = [module.metaflow]
}

output "metaflow_service_sg_id" {
  value       = data.aws_security_group.metaflow_service_sg.id
  description = "Security Group for Metaflow internal service URL"
}

output "additional_bucket_arns" {
  value = var.add_bucket_arn
}